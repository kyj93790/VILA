...
def main(cfg):
    ...
    for name, param in ds_model.named_parameters():
        if 'lora' in name:
            nn.init.normal_(param, mean=0.0, std=cfg.sigma)
    ...
    for epochs in range(10): # use 1-epoch importance measurement for now
        for step, inputs in tqdm(enumerate(trainer.get_train_dataloader())):
            ...
            for n, lp in ds_model.named_parameters():
                if n in grad_f:
                    grad_f[n] += (lp.grad * cnt).detach().cpu()
                    square_grad_f[n] += (lp.grad.pow(2) * cnt).detach().cpu()
                lp.grad = None
            f_cnt += cnt
            ...
            for n, lp in ds_model.named_parameters():
                if n in grad_r:
                    grad_r[n] += (lp.grad * cnt).detach().cpu()
                    square_grad_r[n] += (lp.grad.pow(2) * cnt).detach().cpu()
                lp.grad = None
            r_cnt += cnt
    ...
    torch.save(importances, f"{cfg.save_dir}.pt")
