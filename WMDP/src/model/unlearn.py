...
class Unlearn:
    ...
    def init_dataset(self):
        ...
        print("#######################################################")
        print(f"total batch size:", self.batch_size * self.gradient_accumulation_steps * self.num_devices)
        if self.max_steps == -1:
    ...
    def init_unlearner(self, logger):
        ...
        lora_rank = 8
        if "FILA" in self.unlearn_method:
        ...
        if "VILA" in self.unlearn_method:
            print(f'VILA in unlearn method ------- loading importance file from {self.importance_file}')
            ...
            importance_r = imp_file['importance_r']
            importances = {}
            print(f"len(importance_f): {len(importance_f)}")
            print(f"len(importance_r): {len(importance_r)}")
    ...
    def eval_step(self, logger, step):
        
        if not self.use_lora:
            print("LoRA is not used. Please set use_lora to True.")
            return
        
        root = logger.get_root()

        model_path = os.path.join(root, "unlearn_checkpoint")
        adapter_path = os.path.join(model_path, f"checkpoint-{step}")
        merged_model_path = os.path.join(root, "merged_model")
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(residual_model_path)

        # get peft model
        peft_model = PeftModel.from_pretrained(residual_model, adapter_path)
        ...
        self.model = None
        torch.cuda.empty_cache()
        print(f"Evaluating merged model at {merged_model_path}...")
        eval_few_shots(model_name=merged_model_path, task_list=["mmlu"], output_path=f"{root}/mmlu_step_{step}.json")
        
        torch.cuda.empty_cache()
        eval_few_shots(model_name=merged_model_path, task_list=["wmdp_bio"], output_path=f"{root}/wmdp_bio_step_{step}.json")

        if self.dataset_names['forget'] == "WMDPALL":
            torch.cuda.empty_cache()   
            eval_few_shots(model_name=merged_model_path, task_list=["wmdp_cyber"], output_path=f"{root}/wmdp_cyber_step_{step}.json")
        os.system(f"rm -rf {logger.get_root()}/merged_model")
    ...
    def run(self, logger):
        if self.resume_path is None:
            ...
            if self.use_lora:
                self.save_residual(logger)
            else:
                self.save_full(logger)
            
            eval_steps = list(range(self.per_step, self.max_steps + 1, self.per_step))
            for step in eval_steps:
                self.eval_step(logger, step)
