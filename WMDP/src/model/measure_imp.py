...
class Relearn:
    ...
    def init_model(self):
        ...
        if self.use_lora:
            ...
            if "VILA" in self.unlearn_method:
                print("FOR VILA, reinit lora params")
                for name, param in model.named_parameters():
                    if 'lora' in name:
                        print(f'Reinit param: {name}')
    ...
    def init_dataset(self):
        ...
        # 500 samples for measuring importances
        if self.max_steps == -1:
    ...
    def measure_importances_fila(self, logger):
        print("------------------------- Measure importances for FILA ------------------------- ")
        ...
        for epoch in range(10):
            for step, inputs in tqdm(enumerate(self.unlearner.get_train_dataloader())):
                ...
                forget_outputs.loss.backward()
                cnt = torch.sum(forget_inputs['labels'] != -100)

                for n, lp in self.model.named_parameters():
                    if n in importance_f:
                        importance_f[n] += (lp.grad.pow(2) * cnt.to(lp.grad.device)).detach().cpu()
                    lp.grad = None
                f_cnt += cnt
                
                retain_outputs = self.model(**retain_inputs)
                retain_outputs.loss.backward()
                cnt = torch.sum(retain_inputs['labels'] != -100)

                for n, lp in self.model.named_parameters():
                    if n in importance_r:
                        importance_r[n] += (lp.grad.pow(2) * cnt.to(lp.grad.device)).detach().cpu()
        ...
        torch.save(importances, f"{self.save_path}/wmdp_fila.pt")
    

    def measure_importances_vila(self, logger):
        print("------------------------- Measure importances for VLIA ------------------------- ")
        lora_modules = ['lora_A', 'lora_B']

        # Force all parameters to require gradients
        self.model.train()
        for name, param in self.model.named_parameters():
            for t in lora_modules:
                if t in name:
                    param.requires_grad = True
        ...
        for name, param in self.model.named_parameters():
            for t in lora_modules:
                if t in name:
                    grad_f[name] = 0
                    grad_r[name] = 0
                    square_grad_f[name] = 0
                    square_grad_r[name] = 0
        ...
        for epoch in range(10): 
            for step, inputs in tqdm(enumerate(self.unlearner.get_train_dataloader())):
                ...
                for n, lp in self.model.named_parameters():
                    if n in grad_f:
                        grad_f[n] += (lp.grad * cnt.to(lp.grad.device)).detach().cpu()
                        square_grad_f[n] += (lp.grad.pow(2) * cnt.to(lp.grad.device)).detach().cpu()
                    lp.grad = None
                f_cnt += cnt
                ...
                for n, lp in self.model.named_parameters():
                    if n in grad_r:
                        grad_r[n] += (lp.grad * cnt.to(lp.grad.device)).detach().cpu()
                        square_grad_r[n] += (lp.grad.pow(2) * cnt.to(lp.grad.device)).detach().cpu()
                    lp.grad = None
                r_cnt += cnt
        ...
        for name, param in self.model.named_parameters():
            for t in lora_modules:
                if t in name:
                    importance_f[name] = torch.clamp(((square_grad_f[name] / f_cnt.to(square_grad_f[name].device)) \
                                            - (grad_f[name] / f_cnt.to(grad_f[name].device)).pow(2)), min=0).detach().cpu()
                    importance_r[name] = torch.clamp(((square_grad_r[name] / r_cnt.to(square_grad_r[name].device)) \
                                            - (grad_r[name] / r_cnt.to(grad_r[name].device)).pow(2)), min=0).detach().cpu()
        ...
        torch.save(importances, f"{self.save_path}/wmdp_vila_final.pt")
    ...
    def run(self, logger):
        if self.resume_path is None:
            ...
            if self.unlearn_method == "FILA":
                self.measure_importances_fila(logger)
            elif self.unlearn_method == "VILA":
                self.measure_importances_vila(logger)
        else:
            self.init_model()
            self.init_dataset()
