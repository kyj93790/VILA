...
Section("unlearn.IHL", "IHL unlearning configs").enable_if(
    lambda cfg: cfg["unlearn.unlearn_method"] == "IHL"
).params(
    gamma=Param(float, default=0.0, desc="hyperparameters before IHL loss"),
)

### FILA ###
Section("unlearn.IHL+FILA", "IHL+FILA unlearning configs").enable_if(
    lambda cfg: cfg["unlearn.unlearn_method"] == "IHL+FILA"
).params(
    gamma=Param(float, default=1.0, desc="hyperparameters before IHL loss"),
    importance_file=Param(str, default=None, desc="Importance file"),
)
Section("unlearn.GA+FT+FILA", "GA+FT+FILA unlearning configs").enable_if(
    lambda cfg: cfg["unlearn.unlearn_method"] == "GA+FT+FILA"
).params(
    gamma=Param(float, default=1.0, desc="hyperparameters before GA+FT loss"),
    importance_file=Param(str, default=None, desc="Importance file"),
)
Section("unlearn.NPO+FT+FILA", "NPO+FT+FILA unlearning configs").enable_if(
    lambda cfg: cfg["unlearn.unlearn_method"] == "NPO+FT+FILA"
).params(
    gamma=Param(float, default=1.0, desc="hyperparameters before NPO+FT+FILA loss"),
    beta=Param(float, default=1.0, desc="hyperparameters under NPO+FT+FILA loss"),
    importance_file=Param(str, default=None, desc="Importance file"),
)

### add for VARLFILA ###
Section("unlearn.IHL+VARLFILA", "IHL+VARLFILA unlearning configs").enable_if(
    lambda cfg: cfg["unlearn.unlearn_method"] == "IHL+VARLFILA"
).params(
    gamma=Param(float, default=1.0, desc="hyperparameters before IHL+VARLFILA loss"),
    importance_file=Param(str, default=None, desc="Importance file"),
)
Section("unlearn.GA+FT+VARLFILA", "GA+FT+VARLFILA unlearning configs").enable_if(
    lambda cfg: cfg["unlearn.unlearn_method"] == "GA+FT+VARLFILA"
).params(
    gamma=Param(float, default=1.0, desc="hyperparameters before GA+FT+VARLFILA loss"),
    importance_file=Param(str, default=None, desc="Importance file"),
)
Section("unlearn.NPO+FT+VARLFILA", "NPO+FT+VARLFILA unlearning configs").enable_if(
    lambda cfg: cfg["unlearn.unlearn_method"] == "NPO+FT+VARLFILA"
).params(
    gamma=Param(float, default=1.0, desc="hyperparameters before NPO+FT+VARLFILA loss"),
    beta=Param(float, default=1.0, desc="hyperparameters under NPO+FT+VARLFILA loss"),
    importance_file=Param(str, default=None, desc="Importance file"),
)
### add for VARFILA ###
Section("unlearn.IHL+VARFILA", "IHL+VARFILA unlearning configs").enable_if(
    lambda cfg: cfg["unlearn.unlearn_method"] == "IHL+VARFILA"
).params(
    gamma=Param(float, default=1.0, desc="hyperparameters before IHL+VARFILA loss"),
    importance_file=Param(str, default=None, desc="Importance file"),
)
Section("unlearn.GA+FT+VARFILA", "GA+FT+VARVARFILALFILA unlearning configs").enable_if(
    lambda cfg: cfg["unlearn.unlearn_method"] == "GA+FT+VARFILA"
).params(
    gamma=Param(float, default=1.0, desc="hyperparameters before GA+FT+VARFILA loss"),
    importance_file=Param(str, default=None, desc="Importance file"),
)
Section("unlearn.NPO+FT+VARFILA", "NPO+FT+VARFILA unlearning configs").enable_if(
    lambda cfg: cfg["unlearn.unlearn_method"] == "NPO+FT+VARFILA"
).params(
    gamma=Param(float, default=1.0, desc="hyperparameters before NPO+FT+VARFILA loss"),
    beta=Param(float, default=1.0, desc="hyperparameters under NPO+FT+VARFILA loss"),
    importance_file=Param(str, default=None, desc="Importance file"),
)

Section("unlearn.IHLCKA", "IHLCKA unlearning configs").enable_if(
    lambda cfg: cfg["unlearn.unlearn_method"] == "IHLCKA"
).params(
    gamma=Param(float, default=1.0, desc="hyperparameters before IHLCKA"),
)
...
class Main:
    ...
    def init_model(self, model_name):
        ...
        self.model = import_module(f"model.unlearn_one_step").get(**kwargs)
