...
Section("unlearn", "Unlearning configs").params(
    unlearn_method=Param(
        OneOf(
            [
                "FILA", # Measure Importance FILA
                "VILA", # Measure Importance VILA
            ]
        ),
        default="MIF",
        desc="Measure importances method",
    ),
...
Section("unlearn.FILA", "FILA unlearning configs").enable_if(
    lambda cfg: cfg["unlearn.unlearn_method"] == "MILFILAF"
).params(
    save_path=Param(Folder(True), default="files/importance", desc="save path under FILA importances"),)
Section("unlearn.VILA", "VILA unlearning configs").enable_if(
    lambda cfg: cfg["unlearn.unlearn_method"] == "VILA"
).params(
    save_path=Param(Folder(True), default="files/importance", desc="save path under VILA importances"),
)
...
class Main:
    ...
    def init_model(self, model_name):
        ...
        self.model = import_module(f"model.measure_imp").get(**kwargs)
