...
### add for VILA ###
Section("unlearn.IHL+VILA", "IHL+VILA unlearning configs").enable_if(
    lambda cfg: cfg["unlearn.unlearn_method"] == "IHL+VILA"
).params(
    gamma=Param(float, default=1.0, desc="hyperparameters before IHL+VILA loss"),
    importance_file=Param(str, default=None, desc="Importance file"),
)
Section("unlearn.GA+FT+VILA", "GA+FT+VILA unlearning configs").enable_if(
    lambda cfg: cfg["unlearn.unlearn_method"] == "GA+FT+VILA"
).params(
    gamma=Param(float, default=1.0, desc="hyperparameters before GA+FT+VILA loss"),
    importance_file=Param(str, default=None, desc="Importance file"),
)
Section("unlearn.NPO+FT+VILA", "NPO+FT+VILA unlearning configs").enable_if(
    lambda cfg: cfg["unlearn.unlearn_method"] == "NPO+FT+VILA"
).params(
    gamma=Param(float, default=1.0, desc="hyperparameters before NPO+FT+VILA loss"),
    beta=Param(float, default=1.0, desc="hyperparameters under NPO+FT+VILA loss"),
    importance_file=Param(str, default=None, desc="Importance file"),
)
